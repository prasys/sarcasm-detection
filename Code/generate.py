...
training_path='snippets.xlsx'		#tweets.xlsx for tweets dataset
...
if(run_flag==1):
    test_sents=train_sents[3*int(F):]
    ts=train_sents[:3*int(F)]
    # tss=train_sents[2*F:]
    # ts.extend(tss)
    train_sents=ts

    test_targs=train_targs[3*int(F):]
    ts3=train_targs[:3*int(F)]
    # tss=train_targs[2*F:]
    # ts3.extend(tss)
    train_targs=ts3
